...
**Status:** DONE (2026-07-10 code; 2026-07-13 DEPLOYED — live at https://tfa-books-production.up.railway.app)
...
## Waiting on Chris (all resolved 2026-07-13)
...
- 2026-07-13 worker: deployed via Railway CLI with an account token from Chris (project TFA-Books: Postgres + tfa-books service linked to the GitHub repo/branch, so pushes auto-deploy; DATABASE_URL referenced, APP_PASSWORD set, domain generated). Verified live: login page 200, every protected route 307s to /login, and the app booting proves `prisma migrate deploy` succeeded. Chris was told to revoke the CLI token and can rotate APP_PASSWORD in the service Variables.
