...
**As of 2026-07-13 — deployed, passwordless, and template-equipped.** Goals 011 (magic-link login) and 012 (CoA templates) shipped after the parity run. TFA Books now covers the core of a real accounting package, all of it gated: **79 unit + 65 Playwright browser tests** via `npm run gates`, green from a clean checkout.
...
- **App**: Wave-style shell (sidebar, dashboard with KPIs + monthly chart), **passwordless email magic-link login** against an authorized-user allowlist (Settings → Users; chrism@pictureline.com seeded; sessions HMAC-signed with SESSION_SECRET; without RESEND_API_KEY links print to Railway logs), pg_dump backup/restore with a restore-proving test, Railway deploy config + click-by-click [docs/deploy.md](https://github.com/chrisbachmaxwell/TFA-Quickbooks/blob/claude/project-factory-setup-rz0utl/docs/deploy.md).
- **Chart-of-accounts templates**: three QuickBooks-style standard charts (holding/investment, general small business, rental property) with a picker on empty books and a "Suggested accounts" section on existing ones.
...
## Waiting on Chris
- **RESEND_API_KEY** (resend.com) so sign-in links arrive by email; until then they print to the Railway service Logs tab.
- Revoke the Railway CLI token when done asking for deploys.
